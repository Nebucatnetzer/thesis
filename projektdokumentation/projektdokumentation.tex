...
\label{sec:org731e1a3}
\subsection{Vision}
\label{sec:orgfe6b059}
...
ist. Wie in Abbildung:(\ref{fig:orgeda5bf4}) zu sehen ist, werden die Aktionen effektiv
...
\hline
\end{tabular}
\caption{\label{tab:orgd153ce1}
Kostenrechnung}

\end{table}

\subsection{Projektrisiken}
\label{sec:orgff876a1}
...
\label{sec:org60cc8a5}

In der Tabelle: (\ref{tab:org51a4574}), sind die Risiken des Projektes
gemeinsam mit ihren Gegenmassnahmen aufgelistet. Jedes Risiko wurde
entsprechend der Tabelle: (\ref{tab:org82d6ae9}) nach der Wahrscheinlichkeit
des Eintreffens bewertet und entsprechend der Tabelle: (\ref{tab:org5fbd2d6}) nach
...
\hline
\caption{\label{tab:org3d54c4e}
Projektrisiken}
\\
\end{longtable}

\section{Analyse}
\label{sec:org49a3b73}
\subsection{Umweltanalyse}
\label{sec:org7988060}
...
Tabelle:(\ref{tab:orgc6f0b4e}) wurden die Anforderungen und Wünsche
...
Abbildung:(\ref{fig:orgbc1d314}) grafisch dargestellt.
...
Tabelle: (\ref{tab:org82d6ae9}) nach der Wahrscheinlichkeit des Eintreffens
bewertet und entsprechend der Tabelle: (\ref{tab:org5fbd2d6}) nach seiner Auswirkung
im Bezug auf die Nützlichkeit der gemachten Backups.

In der Tabelle: (\ref{tab:org51a4574}) sind dabei die Risiken für das
Szenario aufgelistet und nummeriert. In der Abbildung:(\ref{fig:org54045c9}), ist die
Bewertung des Ist-Risikos grafisch dargestellt und in der
Abbildung:(\ref{fig:org521926b}), ist das Soll-Risiko welches mit dieser Arbeit
...
\caption{\label{fig:org521926b}
Grafische Darstellung der Soll-Risikoanalyse}
\end{figure}

\newpage
\subsection{SWOT-Analyse}
\label{sec:org23a1f62}
...
Die nachfolgend aufgeführten Use Cases, Tabellen:(\ref{tab:org10be5a1}, \ref{tab:org4fe80d8},
\ref{tab:orgab70314}, \ref{tab:orgf4f8321}, \ref{tab:orgf240273}, \ref{tab:orgdcbae89}, \ref{tab:org336605d})
wurden dem Anwendungsfalldiagramm, Abbildung:(\ref{fig:org1057174}), entnommen und
zusätzlich noch um jeweils ein Aktivitätsdiagramm, Abbildungen:
(\ref{fig:orge7c2d4c}, \ref{fig:org82c09db}, \ref{fig:orgfb3e0f0},
\ref{fig:org92ca552}, \ref{fig:org915780c}, \ref{fig:org08367e9}), erweitert
...
\caption{\label{fig:org08367e9}
Aktivitätsdiagramm zum Aktivieren von automatischen Backups}
\end{figure}
\newpage
\section{Konzept}
\label{sec:org7c1c6e2}

\subsection{Varianten}
\label{sec:orgef5d7e8}
...
der Tabelle Projektziele (\ref{tab:orgb950580}).
...
\hline
\end{tabular}
\caption{\label{tab:org4e0b79a}
Muster Bewertungstabelle}

\end{table}

\subsubsection{Backend}
\label{sec:org351b4d8}

Fürs Backend bieten sich die folgende drei Sprachen an: \hyperref[sec:org6dc83fa]{C\#}, \hyperref[sec:org9726297]{C++}, \hyperref[sec:org88671d5]{Python}.
Dies vor allem, weil alle Allrounder Sprachen sind und sich gut für Desktop
Applikationen eignen.

\paragraph{C\#}
\label{sec:org6dc83fa}
...
\hline
\end{tabular}
\caption{\label{tab:org5f3234b}
C\# Bewertungstabelle}

\end{table}

\paragraph{C++}
\label{sec:org9726297}
...
\hline
\end{tabular}
\caption{\label{tab:org9de043b}
C++ Bewertungstabelle}

\end{table}

\paragraph{Python}
\label{sec:org88671d5}
...
einfachen Befehl, Code Snippet (\ref{org21e4047}) starten.

\lstset{language=bash,label=org21e4047,caption={Minimal Python Setup},captionpos=b,numbers=none}
...
\hline
\end{tabular}
\caption{\label{tab:org09176af}
Python Bewertungstabelle}

\end{table}

\subsubsection{Frontend}
\label{sec:orgf48e554}

Fürs Frontend sind folgende Projekte interessant: \hyperref[sec:orge6f1dad]{Qt}, \hyperref[sec:orgb8f4e3d]{Gtk} und \hyperref[sec:org159ca63]{Electron}. Alle
...
\hline
\end{tabular}
\caption{\label{tab:orgbd7bf27}
Qt Bewertungstabelle}

\end{table}

\paragraph{Gtk}
\label{sec:orgb8f4e3d}
...
Abbildung (\ref{fig:org1567490}).

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{pictures/meld.png}
\caption{\label{fig:org1567490}
...
\hline
\end{tabular}
\caption{\label{tab:org23e94b9}
Gtk Bewertungstabelle}

\end{table}

\paragraph{Electron}
\label{sec:org159ca63}
...
\hline
\end{tabular}
\caption{\label{tab:org9bb7ec2}
Electron Bewertungstabelle}

\end{table}

\subsubsection{Ergebnis}
\label{sec:org6c303b3}

Aufgrund der erreichten Punktzahl, Tabelle:(\ref{tab:org6208ba1}), bei den vorhergehenden
...
Tabelle:(\ref{tab:org7421cce}), final jedoch von Hand überprüft. Somit kann vermieden
...
der Sektion: \hyperref[sec:org7f34c6e]{Releases} beschrieben. Somit werden die Tests immer gegen eine
...
Linux und OS X. Wie in den Screenshots in Abbildung:(\ref{fig:orgc389f8e}) zu sehen
ist.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{pictures/hello_world.png}
\caption{\label{fig:orgc389f8e}
Python und Qt Applikation unter Windows (links), Linux (rechts) und OS X (unten)}
\end{figure}

\subsection{Benutzerinterface}
\label{sec:orgdec9bd7}
\paragraph{Inspiration}
\label{sec:org43aedbb}

PLACEHOLDER

\paragraph{Frontend Umsetzung}
\label{sec:orgc7c5ae3}

PLACEHOLDER

\paragraph{Backend Umsetzung}
\label{sec:org817e067}

PLACEHOLDER

\subsection{Releases}
\label{sec:org7f34c6e}

\section{Ausblick}
\label{sec:org2dfa673}
\subsection{Projektmanagement}
\label{sec:org552185a}

PLACEHOLDER

\subsection{Umsetzung}
\label{sec:org365accc}

PLACEHOLDER

\subsection{Gelerntes}
\label{sec:org33a52a9}
